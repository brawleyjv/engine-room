...
CREATE TABLE `users` (
  ...
  `full_name` varchar(100) GENERATED ALWAYS AS (CONCAT(FirstName, ' ', LastName)) STORED,
  `role` varchar(50) DEFAULT 'user',
  `IsAdmin` tinyint(1) DEFAULT 0,
  `IsActive` tinyint(1) DEFAULT 1,
  `is_support_user` tinyint(1) DEFAULT 0 COMMENT 'Flag for LogicDock support team access',
  ...
  KEY `idx_support_user` (`is_support_user`),
  KEY `idx_role` (`role`)
