    ...
    switch ($step) {
        ...
        case 2:
            ...
            if ($result['success']) {
                ...
                header('Location: signup_test.php?step=3');
                exit;
            } else {
                $errors = $result['errors'];
            }
        ...
        case 3:
            ...
            if ($result['success']) {
                ...
                if ($creation_result['success']) {
                    ...
                    header('Location: welcome.php?new=1');
                    exit;
                } else {
            ...
            <?php switch ($step): 
                ...
                case 2: ?>
                    ...
                    <form method="POST">
                        ...
                        <div class="text-end">
                            <a href="signup_test.php?step=1" class="btn btn-outline-secondary me-2">
                ...
                case 3: ?>
                    ...
                    <form method="POST">
                        ...
                        <div class="text-end">
                            <a href="signup_test.php?step=2" class="btn btn-outline-secondary me-2">
                                <i class="fas fa-arrow-left me-2"></i>Back
                            </a>
...
function handleStep1($data) {
    ...
    if (empty($data['company_domain']) || !preg_match('/^[a-z0-9-]+$/', $data['company_domain'])) {
        $errors[] = 'Company domain must contain only lowercase letters, numbers, and hyphens';
    }
    ...
    if (!empty($data['company_domain'])) {
        ...
        if (!$existing) {
            $errors[] = 'This domain is already taken. Please choose another.';
        }
    }
...
function checkDomainAvailability($domain) {
    ...
    try {
        ...
        if ($license_conn->connect_error) {
            throw new Exception("License database connection failed");
        }
        ...
        return $result->num_rows === 0; // Available if no rows found
...
function createCompanyDatabase($data) {
    ...
    try {
        ...
        // Create database name (safe for MySQL)
        $db_name = 'vessel_' . preg_replace('/[^a-z0-9]/', '_', $data['company_domain']);
        
        // Create the company database
        $license_conn->query("CREATE DATABASE IF NOT EXISTS `$db_name`");
        
        // Insert company record
        $stmt = $license_conn->prepare("
            INSERT INTO companies (
                ...
                subscription_plan, subscription_status, trial_end_date, created_at
            ) VALUES (?, ?, ?, ?, 'localhost', ?, 'vessel_user', 'vessel_pass', ?, 'active', DATE_ADD(NOW(), INTERVAL 30 DAY), NOW())
        ");
        ...
        $stmt->execute();
        $company_id = $license_conn->insert_id;
        
        // Set up the company database schema
        $company_conn = new mysqli('localhost', 'root', '', $db_name);
        
        if ($company_conn->connect_error) {
            throw new Exception("Company database connection failed");
        }
        
        // Create basic schema (separated for proper execution)
        $schema_queries = [
            "CREATE TABLE users (
                ...
                role ENUM('admin', 'captain', 'engineer', 'crew') DEFAULT 'crew',
        ...
        foreach ($schema_queries as $query) {
            if (!$company_conn->query($query)) {
                throw new Exception("Schema creation failed: " . $company_conn->error);
            }
        }
        
        // Insert admin user
        $hashed_password = password_hash($data['admin_password'], PASSWORD_DEFAULT);
        $stmt = $company_conn->prepare("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, 'admin')");
        $stmt->bind_param("sss", $data['admin_name'], $data['admin_email'], $hashed_password);
        $stmt->execute();
        $user_id = $company_conn->insert_id;
        
        // Create support user
        $support_password = password_hash(generatePassword(12), PASSWORD_DEFAULT);
        $stmt = $company_conn->prepare("INSERT INTO users (name, email, password, role) VALUES ('Support User', ?, ?, 'admin')");
        $support_email = 'support@' . $data['company_domain'] . '.vessellogger.com';
        $stmt->bind_param("ss", $support_email, $support_password);
        $stmt->execute();
