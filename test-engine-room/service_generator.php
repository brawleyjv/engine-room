<?php
/**
 * Generator Service Management Page
 * Track and manage service items for individual generators
 */
...
// Get generator type from URL
$generator_type = $_GET['type'] ?? '';
if (empty($generator_type)) {
...
$is_active = isGeneratorActive($generator_type, $vessel_settings);
if (!$is_active) {
    header('Location: index.php');
    exit;
}
...
if ($_POST && isset($_POST['complete_service'])) {
    try {
        ...
        $service_stmt = $pdo->prepare("
            ...
            WHERE si.item_code = ? AND ss.equipment_type = 'generator' AND ss.equipment_id = ?
        ");
        $service_stmt->execute([$service_code, $generator_type]);
        ...
        if ($service_item) {
            ...
            $log_message = "Service completed: {$service_item['item_name']} on " . 
                          ucwords(str_replace('_', ' ', $generator_type)) . " Generator at {$current_hours} hours. " .
                          "Next service due at {$next_service_hours} hours.";
            if (!empty($notes)) {
                $log_message .= " Notes: " . $notes;
            }
            
            $log_id = createLogEntry($pdo, 'service', $generator_type, $log_message, $performed_by);
...
// Get current generator hours
$hours_stmt = $pdo->prepare("SELECT total_hours FROM generator_hours WHERE generator_type = ?");
$hours_stmt->execute([$generator_type]);
$generator_data = $hours_stmt->fetch(PDO::FETCH_ASSOC);
$current_generator_hours = $generator_data ? $generator_data['total_hours'] : 0;

// Get service items for this generator
$services_stmt = $pdo->prepare("
    ...
    LEFT JOIN service_tracking st ON si.item_code = st.service_item_code 
                                  AND st.equipment_type = 'generator' 
                                  AND st.equipment_id = ?
    WHERE ss.equipment_type = 'generator' 
      AND ss.equipment_id = ? 
      AND ss.is_enabled = 1
...
$services_stmt->execute([
    $current_generator_hours, $current_generator_hours, $current_generator_hours,
...
$generator_name = ucwords(str_replace('_', ' ', $generator_type)) . ' Generator';
?>
...
<head>
    ...
    <title><?php echo $generator_name; ?> Service Management - Marine Engine Room</title>
    <link rel="stylesheet" href="test_styles.css">
</head>
<body>
    <div class="header">
        <h1>⚡ <?php echo $generator_name; ?> Service Management</h1>
        <p>Track and manage service intervals for generator maintenance items</p>
    ...
    <div class="container">
        ...
        <div class="form-section" style="background: #f8f9fa; border: 1px solid #dee2e6;">
            <h3>📊 <?php echo $generator_name; ?> Service Status</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
                <div style="text-align: center;">
                    <div style="font-size: 24px; font-weight: bold; color: #28a745;">
                        <?php echo number_format($current_generator_hours, 1); ?>
                    </div>
                    <div style="font-size: 14px; color: #6c757d;">Total Generator Hours</div>
                </div>
                <?php 
                            ...
                            <div class="service-item-card" style="border: 2px solid <?php echo $status_colors[$service['status']]; ?>; border-radius: 8px; padding: 15px; background: white;">
                                    ...
                                    <div>
                                        ...
                                        <span style="color: #6c757d;">
                                            ...
                                            if ($service['next_service_hours']) {
                                                $hours_until = $service['next_service_hours'] - $current_generator_hours;
                                                if ($hours_until <= 0) {
                                ...
                                <form method="post" style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e9ecef;">
                                    <div style="display: grid; grid-template-columns: 120px 200px 1fr auto; gap: 10px; align-items: end;">
                                        <div>
                                            <label style="font-size: 11px; color: #495057;">Generator Hours:</label>
                                            <input type="number" name="current_hours" value="<?php echo $current_generator_hours; ?>" 
        ...
        <?php else: ?>
            <div class="form-section" style="text-align: center; padding: 40px;">
                ...
                <p>No service items are currently enabled for this generator.</p>
        ...
        <div style="margin: 30px 0; text-align: center;">
            <a href="manage_generator.php?type=<?php echo $generator_type; ?>" 
