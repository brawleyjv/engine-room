<?php
/**
 * Engine Service Management Page
 * Track and manage service items for individual engines
 */
...
// Get engine type from URL
$engine_type = $_GET['type'] ?? '';
...
$is_active = isEngineActive($engine_type, $vessel_settings);
if (!$is_active) {
    header('Location: index.php');
    exit;
}
...
if ($_POST && isset($_POST['complete_service'])) {
    try {
        ...
        $service_stmt = $pdo->prepare("
            ...
            WHERE si.item_code = ? AND ss.equipment_type = 'engine' AND ss.equipment_id = ?
        ");
        $service_stmt->execute([$service_code, $engine_type]);
        ...
        if ($service_item) {
            ...
            $log_message = "Service completed: {$service_item['item_name']} on " . 
                          ucwords(str_replace('_', ' ', $engine_type)) . " at {$current_hours} hours. " .
                          "Next service due at {$next_service_hours} hours.";
            if (!empty($notes)) {
                $log_message .= " Notes: " . $notes;
            }
            
            $log_id = createLogEntry($pdo, 'service', $engine_type, $log_message, $performed_by);
...
$hours_stmt = $pdo->prepare("SELECT total_hours FROM engine_hours WHERE engine_type = ?");
$hours_stmt->execute([$engine_type]);
$engine_data = $hours_stmt->fetch(PDO::FETCH_ASSOC);
$current_engine_hours = $engine_data ? $engine_data['total_hours'] : 0;

// Get service items for this engine
...
$services_stmt->execute([
    $current_engine_hours, $current_engine_hours, $current_engine_hours,
...
<head>
    ...
    <title><?php echo $engine_name; ?> Service Management - Marine Engine Room</title>
    <link rel="stylesheet" href="test_styles.css">
</head>
<body>
    <div class="header">
        <h1>🔧 <?php echo $engine_name; ?> Service Management</h1>
        <p>Track and manage service intervals for engine maintenance items</p>
    ...
    <div class="container">
        ...
        <div class="form-section" style="background: #f8f9fa; border: 1px solid #dee2e6;">
            <h3>📊 <?php echo $engine_name; ?> Service Status</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
                <div style="text-align: center;">
                    <div style="font-size: 24px; font-weight: bold; color: #007bff;">
                        <?php echo number_format($current_engine_hours, 1); ?>
                    </div>
                    <div style="font-size: 14px; color: #6c757d;">Total Engine Hours</div>
                </div>
                <?php 
                ...
                <div style="text-align: center;">
                    ...
                    <div style="font-size: 14px; color: #6c757d;">Upcoming</div>
                </div>
            </div>
        </div>
                            ...
                            <div class="service-item-card" style="border: 2px solid <?php echo $status_colors[$service['status']]; ?>; border-radius: 8px; padding: 15px; background: white;">
                                    ...
                                    <div>
                                        ...
                                        <span style="color: #6c757d;">
                                            ...
                                            if ($service['next_service_hours']) {
                                                $hours_until = $service['next_service_hours'] - $current_engine_hours;
                                                if ($hours_until <= 0) {
                                ...
                                <form method="post" style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e9ecef;">
                                    <div style="display: grid; grid-template-columns: 120px 200px 1fr auto; gap: 10px; align-items: end;">
                                        <div>
                                            <label style="font-size: 11px; color: #495057;">Engine Hours:</label>
                                            <input type="number" name="current_hours" value="<?php echo $current_engine_hours; ?>" 
        ...
        <?php else: ?>
            <div class="form-section" style="text-align: center; padding: 40px;">
                ...
                <p>No service items are currently enabled for this engine.</p>
        ...
        <div style="margin: 30px 0; text-align: center;">
            <a href="manage_engine.php?type=<?php echo $engine_type; ?>" 
